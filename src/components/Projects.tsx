      ...
      <div className="p-7 sm:p-8 flex-1 flex flex-col justify-between gap-6">
        <div className="space-y-3.5">
          <div className="flex items-start justify-between gap-3">
            <h3
              className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors leading-snug"
              style={{ fontFamily: "Space Grotesk, sans-serif" }}
            >
              {project.title}
            </h3>
            <span className="p-2.5 rounded-xl bg-purple-50 dark:bg-purple-950/60 text-purple-600 dark:text-purple-400 group-hover:bg-purple-600 group-hover:text-white transition-all duration-300 flex-shrink-0">
        ...
        <div className="space-y-5">
          <div className="flex flex-wrap gap-2">
            {project.tech_stack.map((tech) => (
              <span
                key={tech}
                className="px-3 py-1.5 text-xs sm:text-sm font-semibold bg-slate-50 dark:bg-slate-800/80 text-slate-700 dark:text-slate-200 border border-slate-200/70 dark:border-slate-700 rounded-xl group-hover:bg-purple-50/70 dark:group-hover:bg-purple-950/40 group-hover:text-purple-700 dark:group-hover:text-purple-300 transition-colors"
              >
                {tech}
              </span>
            ))}
          ...
          {/* Bottom Actions */}
          <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center gap-3" onClick={(e) => e.stopPropagation()}>
            <button
              onClick={() => window.open(project.live_url, "_blank")}
              className="flex-1 py-3 px-5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white text-sm sm:text-base font-extrabold rounded-xl shadow-md shadow-purple-300/40 dark:shadow-purple-900/40 hover:shadow-purple-400/60 transition-all flex items-center justify-center gap-2 cursor-pointer"
            >
